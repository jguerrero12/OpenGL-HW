...
HW1b::paintGL()
{
    ...
    for(uint i=0, j=0; i<m_colors.size(); ++i) {
        glColor3f(m_colors[i][0], m_colors[i][1], m_colors[i][2]); // select ith color for ith triangle
        
        glBegin(GL_TRIANGLES); // draw ith triangle from the points (verticies) store.
